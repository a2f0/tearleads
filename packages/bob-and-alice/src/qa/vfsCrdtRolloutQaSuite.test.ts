...
import {
  ...
  resolveCandidateSha,
  runQaSuiteChecks
} from './vfsCrdtRolloutQaSuite.js';

describe('vfsCrdtRolloutQaSuite', () => {
    ...
    const { report } = await runQaSuiteChecks(
      {
        ...
        reportJsonPath: 'coverage/qa/vfsCrdtRolloutQaSuite.json',
        reportMarkdownPath: 'coverage/qa/vfsCrdtRolloutQaSuite.md'
      },
      'sha-123',
      async (check) => {

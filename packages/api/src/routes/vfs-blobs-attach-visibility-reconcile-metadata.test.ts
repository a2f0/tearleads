...
describe('VFS routes (blobs attach visibility reconcile metadata)', () => {
  ...
  it('returns 500 when reconcile state has an invalid last_reconciled_at timestamp', async () => {
  ...
  it('returns 500 when reconcile state is missing last_reconciled_change_id', async () => {
    const authHeader = await createAuthHeader();
    const requiredCursor = encodeVfsSyncCursor({
      changedAt: '2026-02-14T10:00:02.000Z',
      changeId: 'desktop-2'

...
describe('VFS CRDT push route processing', { timeout: 15_000 }, () => {
  beforeEach(() => {
    setupCrdtPushRouteTestEnv();
  });
  ...
  it('returns staleWriteId when replica write id is behind', async () => {
    ...
    mockQuery
      ...
      .mockResolvedValueOnce({
        rows: [
          { replica_id: 'desktop', max_write_id: 5, max_occurred_at: null }
        ]
      }) // replica heads
      .mockResolvedValueOnce({ rows: [] }) // existing source
      .mockResolvedValueOnce({}); // COMMIT
  ...
  it('accepts numeric string max_write_id and applies higher writeId', async () => {
    ...
    mockQuery
      ...
      .mockResolvedValueOnce({
        rows: [
          { replica_id: 'desktop', max_write_id: '2', max_occurred_at: null }
        ]
  ...
  it('treats unparsable max_write_id as zero', async () => {
    ...
    mockQuery
      ...
      .mockResolvedValueOnce({
        rows: [
          { replica_id: 'desktop', max_write_id: 'nope', max_occurred_at: null }
        ]
  ...
  it('returns outdatedOp when insert affects zero rows', async () => {
    ...
    mockQuery
      ...
      .mockResolvedValueOnce({
        rows: [
          { replica_id: 'desktop', max_write_id: null, max_occurred_at: null }
        ]

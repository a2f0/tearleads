...
// Bun's module linker resolves transitive exports before vi.mock can intercept
// the @tearleads/shared/redis subpath via the barrel index.js import.
// The dynamic import of ./index.js triggers a SyntaxError under Bun, so the
// entire file must bail out early.
...
if (!isBun) {
  ...
  describe('gracefulShutdown', () => {
    it.skip('skipped under Bun (module linker resolves before vi.mock)', () => {});
  });
}

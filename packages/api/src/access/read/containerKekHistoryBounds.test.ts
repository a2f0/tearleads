...
test.each(["unchanged", "changing"] as const)(
  "same-epoch %s-grant history accepts the bound, refuses overflow, and resumes after rekey",
  async (grantShape) => {
    ...
    await db
      ...
      .set({
        state: { ...overflow.state, containerKeyEpochId: nextKeyEpochId },
      })

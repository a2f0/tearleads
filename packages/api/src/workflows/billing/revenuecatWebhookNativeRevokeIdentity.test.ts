...
test.each(["INITIAL_PURCHASE", "NON_RENEWING_PURCHASE"] as const)(
  "a tokenless native %s cannot create an incomplete binding",
  async (type) => {
...
test.each([
  ...
  "a %s native %s cannot revoke the live subscription",
  async (identity, type) => {
    const admin = createTestUser();
    const organizationId = await registerAndAuthenticate(admin);
    const now = Date.now();
...
test.each(["RENEWAL", "EXPIRATION"] as const)(
  "an exact-token %s cannot bypass missing audit store lineage",
  async (type) => {
    const admin = createTestUser();
    const organizationId = await registerAndAuthenticate(admin);
    const now = Date.now();

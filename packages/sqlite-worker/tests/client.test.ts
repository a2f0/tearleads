...
test("destroy rejects pending requests and detaches listeners", async () => {
  ...
  expect(pendingPing).rejects.toThrow("Database worker client has been destroyed.");
  ...
  expect(client.ping()).rejects.toThrow("Database worker client has been destroyed.");
});

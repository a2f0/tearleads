...
test.each(["before-pass", "in-flight"] as const)(
  "a policy revocation %s cannot advance descendants using stale held membership",
  async (timing) => {
    ...
    try {
      ...
      expect(
        previousPolicy?.projection.map((member) => member.userId),
      ).toContain(USER_ID);
      expect(

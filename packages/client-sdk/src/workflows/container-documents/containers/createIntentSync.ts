...
async function markContainerDocumentsContainerCreateIntentAlreadySynced(input: {
  containerState: ContainerState;
  ...
  const execSql = getContainerDocumentsWorkflowRuntimeExecSql(state.runtime);
...
async function trySyncPendingContainerDocumentsContainerCreateIntent(
  ...
  if (hasRemoteContainerMetadataState(containerState)) {
    await markContainerDocumentsContainerCreateIntentAlreadySynced({
      containerState,
      intent,
      state,
    });
  ...
  state.runtime.log(
    `Container documents: synced local container create ${containerState.container.id}`,
  );
  return "created";
}
...
export async function syncPendingContainerCreateIntents(input: {
  ...
  while (progressed) {
    ...
    for (const intent of pendingIntents) {
      ...
      const result =
        await trySyncPendingContainerDocumentsContainerCreateIntent({
          host,
          intent,
          state,
        });

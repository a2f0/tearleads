...
import type { ContainerDocumentsPersistence } from "../../data/persistence/container-documents/containerDocumentsPersistence";
...
type ContainerMetadataPersistenceRuntime = ContainerDocumentsWorkflowSqlRuntime;
...
async function syncRemoteContainerMetadata(input: {
  ...
  if (!author) {
    runtime.log(
      "Container documents: skipped metadata sync because the writer context is unavailable.",
    );
    return null;
  }
  ...
  const synced = await syncRemoteDocument({
    ...
    if (isStaleContainerMetadataSecurityStateError(error)) {
      runtime.log(
        `Container documents: deferred metadata sync for ${containerId} because its content-key targets are stale.`,
      );
      return null;
    }

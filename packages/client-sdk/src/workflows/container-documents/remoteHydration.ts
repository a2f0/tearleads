...
async function reconcileLocalOnlyRootContainer(input: {
  ...
  state.runtime.log(
    `Container documents: reconciled local root ${localRootState.container.id} into remote root ${remoteRootState.container.id}`,
  );
}
...
export async function hydrateRemoteContainers(input: {
  ...
  if (changedCount > 0) {
    host.updateSnapshot();
    state.runtime.log(
      `Container documents: applied ${changedCount} remote container change(s)`,
    );
  }
}

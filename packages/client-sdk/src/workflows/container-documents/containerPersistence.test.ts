...
type PendingUpdateInput = Parameters<
  ContainerDocumentsPersistence["enqueuePendingUpdate"]
>[1];

function createContainerDocumentsPersistence(input: {
...
test("enqueuePendingContainerUpdateFromRuntime uses the runtime executor", async () => {
  ...
  await enqueuePendingContainerUpdateFromRuntime({
    ...
    persistence: createContainerDocumentsPersistence({ pendingUpdates }),
    runtime: createContainerDocumentsWorkflowSqlRuntime({ execSql }),

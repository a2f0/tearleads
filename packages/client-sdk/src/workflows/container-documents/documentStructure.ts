...
async function primeDocumentStoreForStructuralMutation<TRuntime>({
  ...
  if (!(await documentStore.ensureInitialized())) {
    runtime.log(
      `Container documents: note ${note.id} is not ready to mutate locally`,
    );
    return null;
  }
...
async function relinkContainerDocumentLocally<TRuntime>(params: {
  ...
  if (!relinkedNote) {
    runtime.log(
      `Container documents: note ${note.id} could not be relinked after a structural mutation`,
    );
    return null;
  }
...
export async function moveDocumentLinkState<TRuntime>(params: {
  ...
  if (movedDocument.status === "partial") {
    runtime.log(
      `Container documents: partially moved note ${movedNote.id}; linked to ${nextContainerId} but still linked to ${note.containerId}`,
    );
  } else {
    runtime.log(
      `Container documents: moved note ${movedNote.id} to ${nextContainerId}`,
    );
  }
...
export async function linkDocumentLinkState<TRuntime>(params: {
  ...
  runtime.log(
    `Container documents: linked note ${linkedNote.id} to ${targetContainerId}`,
  );
  return linkedNote;
}
...
export async function unlinkDocumentLinkState<TRuntime>(params: {
  ...
  if (!nextContainerId) {
    runtime.log(
      `Container documents: note ${note.id} has no remaining linked containers after unlink`,
    );
    return null;
  }
  ...
  runtime.log(
    `Container documents: unlinked note ${unlinkedNote.id} from ${removedContainerId}`,
  );
  return unlinkedNote;
}
...
export async function activateDocumentLinkState<TRuntime>(params: {
  ...
  runtime.log(
    `Container documents: switched active note ${relinkedNote.id} to ${targetContainerId}`,
  );
  return relinkedNote;
}

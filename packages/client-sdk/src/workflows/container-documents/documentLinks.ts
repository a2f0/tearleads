...
export async function relinkRemoteContainerDocument(input: {
  ...
  if (!author || !targetSecretKey) {
    runtime.log(
      "Container documents: document mutation skipped because the local key context is unavailable",
    );
    return null;
  }
  ...
  try {
    ...
    if (!result) {
      runtime.log(
        `Container documents: failed to ${operation} note ${noteId} ${operation === "link" ? "to" : "from"} container ${targetContainerId}`,
      );
      return null;
    }
    ...
    runtime.log(
      `Container documents: failed to ${operation} note ${noteId} ${operation === "link" ? "to" : "from"} container ${targetContainerId}: ${error instanceof Error ? error.message : String(error)}`,
    );
    return null;
  }
}
...
export async function moveRemoteContainerDocument(input: {
  ...
  if (!unlinkedDocument) {
    runtime.log(
      `Container documents: note ${noteId} was linked to ${targetContainerId} but failed to unlink from ${currentContainerId}`,
    );

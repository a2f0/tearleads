...
| `documents` | Platform runtime | Document creation, persistence, sync (including the headless `syncRemoteDocument` boundary, whose bounded result reports `hasDeferredPendingUpdates` after durable progress when the caller must schedule another queue pass), projection keys, document link-set helpers, local orphan/blob maintenance, and the discard-to-shell escape hatch for documents whose queued writes can no longer sync. |

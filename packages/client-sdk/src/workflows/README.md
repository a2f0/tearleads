...
Document stores report handled incoming-update quarantines through the optional
`util.logError(message, error)` callback after recording the durable failure.
The `Tearleads` client supplies its host logger; direct runtime consumers can
provide their own. Preserve the original `Error` for host diagnostics and apply
privacy filtering there. Stale generations do not report, and logger failures
do not replace the quarantine.

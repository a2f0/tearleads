...
function generateColumn(
  ...
  if (column.references) {
    ...
    // Self-referencing columns need AnySQLiteColumn type annotation to avoid circular inference
    const isSelfReference = table === currentTableName;
    const returnType = isSelfReference ? ': AnySQLiteColumn' : '';
...
function collectDrizzleTypes(tables: TableDefinition[]): string[] {
  ...
  for (const table of tables) {
    ...
    if (hasSelfReferences(table)) {
      types.add('type AnySQLiteColumn');
    }
  }

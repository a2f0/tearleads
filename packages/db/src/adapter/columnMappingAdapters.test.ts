/**
 * Tests for column name mapping in WebAdapter getConnection() and execute().
 *
 * Verifies that:
 * 1. getConnection() does NOT map column names (Drizzle expects snake_case)
 * 2. execute() returns raw snake_case column names
 * 3. SQL aliases produce camelCase when needed
 * 4. Pre-mapping breaks Drizzle ORM (bug reproduction)
 * 5. Drizzle schema column mapping simulation
 */

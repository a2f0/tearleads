/**
 * Hook for querying VFS items marked as deleted.
...
export interface UseVfsTrashItemsResult {
  items: VfsTrashItem[];
  loading: boolean;
  error: string | null;
  hasFetched: boolean;
...
interface UseVfsTrashItemsOptions {
  enabled?: boolean;
  sort?: VfsSortState;
}
...
export function useVfsTrashItems(
  options: UseVfsTrashItemsOptions = {}
  ...
  const [items, setItems] = useState<VfsTrashItem[]>([]);
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      ...
      const rows = await queryDeletedItems(db, sort);

      const resultItems: VfsTrashItem[] = rows.map((row) => ({
        id: row.id,
      ...
      console.error('Failed to fetch VFS trash items:', err);
  ...
  useEffect(() => {
    ...
    if (sortChanged && hasFetched && isUnlocked && enabled && !loading) {
      fetchItems();
    }
  }, [sort, hasFetched, isUnlocked, enabled, loading, fetchItems]);

/**
 * Hook for querying VFS items that have been shared with the current user.
...
interface UseVfsSharedWithMeOptions {
  enabled?: boolean;
  sort?: VfsSortState;
}
...
export function useVfsSharedWithMe(
  options: UseVfsSharedWithMeOptions = {}
): UseVfsSharedWithMeResult {
  ...
  const [items, setItems] = useState<VfsSharedWithMeItem[]>([]);
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      ...
      const rows = await querySharedWithMe(db, currentUserId, sort);

      const resultItems: VfsSharedWithMeItem[] = rows.map((row) => ({
        id: row.id,
      ...
      console.error('Failed to fetch VFS shared with me items:', err);

...
describe('useVfsSharing integration (real database)', () => {
  ...
  it('useVfsShares normalizes missing orgShares payloads to an empty array', async () => {
    await withRealDatabase(
      async ({ db }) => {
        const itemId = crypto.randomUUID();
        const mockShareApi: VfsShareApiFunctions = {
          getShares: vi.fn(async () => JSON.parse('{"shares": []}')),
        ...
        expect(mockShareApi.getShares).toHaveBeenCalledWith(itemId);
        expect(result.current.shares).toEqual([]);
        expect(result.current.orgShares).toEqual([]);
        expect(result.current.error).toBeNull();
      },
      { migrations: vfsAclEnabledMigrations }
    );

...
The staging wrappers default to `https://api-staging.tearleads.com`; production
wrappers continue to default to `https://api.tearleads.com`.
All wrappers delegate to `scripts/lib/nativeRelease.sh`, while
...
without relying on Xcode's local autogenerated schemes. Android staging inherits
the release build type and uses the same upload keystore as production; its
separate application ID and Play app provide the store isolation.

...
<resources>
    ...
    <string name="custom_url_scheme">${config.bundleIds.android}</string>
</resources>
`;
}

/**
 * Generate gradle.properties fragment for Android build configuration.
 * These can be loaded in build.gradle.
 */
export function generateGradleProperties(config: AppConfig): string {
  return `# Generated by @tearleads/app-builder
# App: ${config.displayName} (${config.id})
# Include this in your build with: apply from: 'app-config.gradle'

app.id=${config.id}
app.displayName=${config.displayName}
app.bundleId=${config.bundleIds.android}
`;
}

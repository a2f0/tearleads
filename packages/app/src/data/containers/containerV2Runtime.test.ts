...
async function signContainerEvent(input: {
  ...
  objectId: string;
  organizationId: string;
  signer: ContainerV2MutationAuthor;
}): Promise<{
  event: AccessEventV2;
  eventHash: string;
}> {
...
async function createContainerManifestFixture(input: {
  ...
  directGrants: readonly ContainerDirectGrantV2[];
  eventId: string;
...
async function createParentProjection(): Promise<{
  ...
  const recipientTargets: ContainerKekRecipientTargetV2[] = [
    {
      recipientKind: "user",
      recipientId: userId,
      recipientKeyEpochId,
  ...
  return {
    ...
    projection: {
      ...
      organizationId,
      path: [
        parentManifest as unknown as ContainerV2WriterProjectionResponse["path"][number],
      ],
      containerKeks: [
        parentKekState as unknown as ContainerV2WriterProjectionResponse["containerKeks"][number],
      ],
    },
...
async function createMutationResponseFromRequest(
  ...
  const body = request.body as ContainerCreateAccessEventBodyV2;
...
test("buildMaterializedContainerV2CreatePlan signs a child create and wraps the child KEK to the parent KEK only", async () => {
  ...
  const materialized = await buildMaterializedContainerV2CreatePlan({
    author,
  ...
  expect(isContainerV2MutationRequest(plan.request)).toBe(true);
...
test("buildContainerV2CreatePlan rejects stale parent projections and wrong organization authors", async () => {
  const parent = await createParentProjection();
    ...
    buildContainerV2CreatePlan({
      ...
      parentProjection: {
        ...
        containerKeks: [
          ...
          } as ContainerV2WriterProjectionResponse["containerKeks"][number],
        ],
      },
    }),
...
test("createRemoteContainerV2 fetches the parent projection and submits the materialized V2 mutation", async () => {
  ...
  const submittedRequests: ContainerV2MutationRequest[] = [];
  const result = await createRemoteContainerV2({
    apiClient: {
      getContainerV2WriterProjection: async (containerId) =>
        containerId === parent.projection.containerId
          ? parent.projection
          : null,
      createContainerV2: async (request) => {
  ...
  expect(isContainerV2MutationRequest(result?.plan.request)).toBe(true);
});

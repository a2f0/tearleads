...
test("applies a persisted local-only preference before the first runtime input", () => {
  ...
  expect(symcrypt.session.syncEnabled).toBe(false);
  expect(symcrypt.runtime.input().state.online).toBe(false);
...
test("marks SDK events disconnected when the WebSocket binding changes URL", async () => {
  ...
  try {
    ...
    expect(symcrypt.events.connectionGeneration).toBe(0);
...
test("derives local keyring blob store keys by namespace without overlapping sessions", async () => {
  ...
  try {
    ...
    symcrypt.blobs.useIdentityNamespace("identity-a");
    const identityAStore = symcrypt.blobs.store;
    symcrypt.blobs.useIdentityNamespace("identity-b");
    const identityBStore = symcrypt.blobs.store;

...
test("canMutateExplorerDocumentLink checks local mutation prerequisites", () => {
  expect(canMutateExplorerDocumentLink(createRuntime())).toBe(true);
  expect(
    canMutateExplorerDocumentLink({
      dbStatus: "loading",
      isAuthenticated: true,
...
test("activateExplorerDocumentLinkState relinks the local document without requesting remote sync", async () => {
  const logs: string[] = [];
  const mergedDocuments: DocumentSummary[] = [];
  const relinkInputs: ExplorerDocumentStructuralMutationRelinkInput[] = [];
  const requestedSyncs: string[] = [];
  const updatedRuntimes: string[] = [];
  const host: ExplorerDocumentStructuralMutationHost<string> = {
  ...
  const activated = await activateExplorerDocumentLinkState({
    host,
    note: createNote(),
  ...
  expect(logs).toEqual([
    "Explorer: switched active note note-1 to container-2",
  ]);
});

test("activateExplorerDocumentLinkState skips documents that are not locally ready", async () => {
  const logs: string[] = [];
  const host: ExplorerDocumentStructuralMutationHost<string> = {
    createDocumentRuntime: (containerId) => `runtime:${containerId}`,
  ...
  expect(logs).toEqual([
    "Explorer: note note-1 is not ready to mutate locally",
  ]);
});

...
const CLIPBOARD_SAFE_LOG_PATTERN =
  /(?:^|: )((?:document priming candidates=\d+ roots=\d+ primed=\d+ unroutable=\d+)|(?:stale root recovery status=(?:ambiguous|context-changed|reassigned|unsupported) candidates=\d+)|(?:interest baseline containers=\d+)|(?:interest declaration acknowledged)|(?:remote revalidation scheduled reason=(?:reconnect|startup))|(?:remote revalidation result=(?:applied incomingUpdates=\d+ attachmentSlots=\d+|unavailable)))$/u;

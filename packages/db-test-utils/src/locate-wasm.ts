 ...
 *
 * The WASM files are downloaded to packages/client/src/workers/sqlite-wasm/
 * by the scripts/downloadSqliteWasm.sh script. This helper finds them
...
const WASM_SUBPATH = 'packages/client/src/workers/sqlite-wasm';
...
export function locateWasmDir(startDir?: string): string {
  ...
  while (currentDir !== root) {
    const candidatePath = path.join(currentDir, WASM_SUBPATH);

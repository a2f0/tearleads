...
export function desktopSentryCommit(root: string): string {
  ...
  // must describe the root itself. Ask Git for its relative prefix instead of
  // comparing path spellings: Windows can return both short and long paths.
  if (git("rev-parse", "--show-prefix").replace(/\r?\n$/, "") !== "")
    throw new Error(

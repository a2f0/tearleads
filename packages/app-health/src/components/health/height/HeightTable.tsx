...
import type { HeightReading } from '../../../lib/healthTrackerTypes.js';

interface HeightTableProps {
...
export function HeightTable({ readings }: HeightTableProps) {
  ...
  return (
    <div className="h-full overflow-auto" data-testid="height-table">
      <table
        className={WINDOW_TABLE_TYPOGRAPHY.table}
        aria-label="Height readings table"
      >
          ...
          {sortedReadings.map((reading) => (
            <WindowTableRow key={reading.id}>
              ...
              <td className={WINDOW_TABLE_TYPOGRAPHY.cell}>
                {formatHeight(reading.value, reading.unit)}
              </td>

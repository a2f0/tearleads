...
export function getMobileGridTemplateColumns(): string {
  return `repeat(${MOBILE_COLUMNS}, minmax(0, 1fr))`;
}

export function getMinSelectionDragDistance(): number {
  return MIN_SELECTION_DRAG_DISTANCE;
}

export function getIconStyleClasses(
  iconDepth?: DesktopIconDepthValue,
  iconBackground?: DesktopIconBackgroundValue
): { iconBgClasses: string; iconFgClass: string } {

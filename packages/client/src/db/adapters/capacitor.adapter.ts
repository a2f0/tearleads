...
export class CapacitorAdapter implements DatabaseAdapter {
  ...
  async exportDatabase(): Promise<Uint8Array> {
    ...
    // Use exportToJson to get the database content as JSON
    // This is the safest cross-platform approach for encrypted databases
  ...
  async importDatabase(
    ...
    let jsonData: JsonSQLite;
    try {
      jsonData = JSON.parse(jsonString) as JsonSQLite;
    } catch {
      throw new Error('Invalid backup file: not valid JSON');
    }

    // Override database name to use our current database
    jsonData.database = this.dbName;
    jsonData.encrypted = true;

    // Import the database using the plugin's native import

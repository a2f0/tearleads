/**
 * Unit tests for OPFSStorage class methods (store, retrieve, delete, exists, etc.).
 */
...
vi.mock('@tearleads/shared', async () => {
  const original = await import('@tearleads/shared');
  return {
    ...original,
...
describe('OPFSStorage class methods', () => {
  ...
  describe('clearAll', () => {
    it('removes all entries from directory', async () => {
      ...
      expect(mockFilesDirectory.removeEntry).toHaveBeenCalledWith('file1.enc');
      expect(mockFilesDirectory.removeEntry).toHaveBeenCalledWith('file2.enc');
    });

...
interface RedisMultiMock {
  ...
  sMembers: (key: string) => RedisMultiMock;
  get: (key: string) => RedisMultiMock;
  expire: (key: string, seconds: number) => RedisMultiMock;
  exec: () => Promise<Array<string | number | string[] | null | undefined>>;
}
  ...
  const client: RedisMockClient = {
    ...
    multi: () => {
      ...
      const chain: RedisMultiMock = {
        ...
        sMembers: (key) => {
          commands.push(async () => client.sMembers(key));
          return chain;
        },
        get: (key) => {
          commands.push(async () => client.get(key));
          return chain;
        },
        expire: (key, seconds) => {
        ...
        exec: async () => {
          const results: Array<string | number | string[] | null | undefined> =
            [];
...
export function createRedisMock(): RedisMockClient {

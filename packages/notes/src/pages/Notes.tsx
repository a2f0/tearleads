...
export function Notes() {
  ...
  const { getDatabase, t, navigateToNote } = useNotesContext();
  const {
    Input,
    ContextMenu,
    ContextMenuItem,
  ...
  const handleNoteClick = useCallback(
    (note: NoteInfo) => {
      navigateToNote?.(note.id, { fromLabel: 'Back to Notes' });
    },
    [navigateToNote]
  );
  ...
  const handleGetInfo = useCallback(() => {
    if (contextMenu) {
      navigateToNote?.(contextMenu.note.id, { fromLabel: 'Back to Notes' });
      setContextMenu(null);
    }
  }, [contextMenu, navigateToNote]);
  ...
  const handleCreateNote = useCallback(async () => {
    try {
      ...
      navigateToNote?.(id, { fromLabel: 'Back to Notes' });
    } catch (err) {
      console.error('Failed to create note:', err);
      setError(err instanceof Error ? err.message : String(err));
    }
  }, [navigateToNote, getDatabase]);

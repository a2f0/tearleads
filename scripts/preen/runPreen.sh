...
run_discovery() {
  case "$1" in
    ...
    preen-deferred-fixes)
      ...
      ;;
    preen-optimize-test-execution)
      sh "$PM_SCRIPT" exec tsx scripts/ciImpact/ciImpact.ts --base origin/main --head HEAD | head -40 || true
      ;;
    preen-database-performance)
    ...
    preen-api-security)
      ...
      ;;
    preen-dependency-security)
      sh "$PM_SCRIPT" audit --prod --audit-level high --json 2>/dev/null | head -40 || true
    ...
    preen-file-limits)
      ...
      ;;
    preen-knip)
      sh "$PM_SCRIPT" exec knip --config knip.ts --use-tsconfig-files --reporter compact | head -80 || true
      ;;
  esac
}
...
metric_count() {
  case "$1" in
    ...
    preen-deferred-fixes)
      ...
      ;;
    preen-optimize-test-execution)
      sh "$PM_SCRIPT" exec tsx scripts/ciImpact/ciImpact.ts --base origin/main --head HEAD 2>/dev/null | jq '.warnings | length' 2>/dev/null || echo 0
      ;;
    preen-database-performance)
    ...
    preen-api-security)
      ...
      ;;
    preen-dependency-security)
      AUDIT_COUNT=$(sh "$PM_SCRIPT" audit --prod --audit-level high --json 2>/dev/null | jq '[.. | objects | .severity? // empty | select(. == "high" or . == "critical")] | length' 2>/dev/null || echo 0)
    ...
    preen-knip)
      KNIP_JSON=$(mktemp)
      sh "$PM_SCRIPT" exec knip --config knip.ts --use-tsconfig-files --reporter json > "$KNIP_JSON" 2>/dev/null || true
      jq '[
        .issues[]? |
        (.dependencies // []),
        (.devDependencies // []),

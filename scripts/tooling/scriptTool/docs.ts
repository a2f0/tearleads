...
import { ACTION_CONFIG, SKILL_INVOKED_ACTIONS } from './actionConfig.ts';
import type { ActionConfig, ActionName, SafetyClass } from './types.ts';
...
function generateReadme(): string {
  ...
  const categories: Record<ActionConfig['category'], ActionName[]> = {
    analysis: [],
    device: [],
    environment: [],
    operations: [],
  ...
  const categoryTitles: Record<ActionConfig['category'], string> = {
    analysis: 'Analysis',
    device: 'Device',
  ...
  const categoryOrder: ActionConfig['category'][] = [
    'analysis',
    'device',
    'environment',
    'operations',
  ...
  const safetyOrder: SafetyClass[] = [
    'safe_read',
    'safe_write_local',
    'safe_write_remote'
  ];
  for (const safetyClass of safetyOrder) {
    const actions = safetyGroups[safetyClass];
    if (actions.length > 0) {
      lines.push(

...
import {
  getNoApiImportSuggestions,
  getNoClientImportSuggestions,
  getSharedUiSuggestions
} from './checkCircularImportsSuggestions.js';
...
function runPackageCycleCheck(repoRoot: string): void {
  ...
  const directApiDependents = [...packageDeps.entries()]
    .filter(
      ([packageName, deps]) =>
        packageName !== API_PACKAGE && deps.has(API_PACKAGE)
    )
    .map(([packageName]) => packageName)
    .sort();

  if (directApiDependents.length > 0) {
    console.error(
      `Error: package imports from ${API_PACKAGE} detected (API app package must not be imported).`
    );
    console.error(
      `Guidance: move shared modules out of ${API_PACKAGE} into ${SHARED_PACKAGE}, @tearleads/db-test-utils, or @tearleads/api-client.`
    );

    for (const dependentPackage of directApiDependents) {
      const edgeKey = `${dependentPackage} -> ${API_PACKAGE}`;
      const reason = edgeReasons.get(edgeKey) ?? 'dependency edge detected';
      console.error(`  - ${edgeKey}: ${reason}`);
    }

    const suggestions = getNoApiImportSuggestions();
    for (const suggestion of suggestions) {

  ...
  try {
    ...
    const inventory = Bun.spawnSync(
      [
        process.execPath,
        "--eval",
        `import { trackedShellScripts } from ${JSON.stringify(modulePath)}; console.log(JSON.stringify(trackedShellScripts(process.cwd())));`,
      ],
...
test("the lint command propagates ShellCheck failures from an extensionless hook", () => {
  const repo = fixture();
  try {

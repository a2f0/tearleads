...
for path in "${files[@]}"; do
  ...
  if [[ "$path" =~ ^packages/[^/]+/package\.json$ ]]; then
    if [ "$path" != "packages/api/package.json" ] && [ -n "$(search_with_line_numbers '"@tearleads/api"[[:space:]]*:' "$path")" ]; then
      violations+=("$path: package dependency on @tearleads/api is not allowed")
    fi
    continue
  fi
...
echo "Error: packages outside packages/api must not import or depend on the API app package." >&2
echo "Move shared contracts/utilities to reusable packages (for example @tearleads/shared, @tearleads/db-test-utils, or @tearleads/api-client)." >&2
printf '%s\n' "${violations[@]}" >&2
exit 1

...
} from "./clientSdkPackageContract";
import { runDependencyCruiserCheck } from "./dependencyCruiserCheck";
...
import { findWorkspaceRegistryViolations } from "./workspaceRegistryContract";
...
const architectureChecks: ArchitectureCheck[] = [
  ...
  createListCheck({
    ...
    message:
      "Every production source file in a registered package should map to exactly one subsystem in scripts/architecture/subsystems.ts. Add the file to the owning subsystem's paths (or a new subsystem) and document it in docs/subsystems.md.",
  ...
  createListCheck({
    ...
    message:
      "The docs/subsystems.md registry table and scripts/architecture/subsystems.ts should list the same subsystems.",

...
sync_dev_env_values() {
  ...
  BLOB_OBJECT_STORE_VALUE="${BLOB_OBJECT_STORE:-s3}"
  BLOB_OBJECT_STORE_S3_BUCKET_VALUE="${BLOB_OBJECT_STORE_S3_BUCKET:-tearleads-blobs}"
  BLOB_OBJECT_STORE_S3_REGION_VALUE="${BLOB_OBJECT_STORE_S3_REGION:-us-east-1}"
  BLOB_OBJECT_STORE_S3_ENDPOINT_VALUE="${BLOB_OBJECT_STORE_S3_ENDPOINT:-http://127.0.0.1:4566}"
  BLOB_OBJECT_STORE_S3_ACCESS_KEY_ID_VALUE="${BLOB_OBJECT_STORE_S3_ACCESS_KEY_ID:-test}"
  BLOB_OBJECT_STORE_S3_SECRET_ACCESS_KEY_VALUE="${BLOB_OBJECT_STORE_S3_SECRET_ACCESS_KEY:-test}"
  BLOB_OBJECT_STORE_S3_FORCE_PATH_STYLE_VALUE="${BLOB_OBJECT_STORE_S3_FORCE_PATH_STYLE:-true}"

  upsert_env_value "${DEV_ENV_FILE}" "BLOB_OBJECT_STORE" "${BLOB_OBJECT_STORE_VALUE}"
  upsert_env_value "${DEV_ENV_FILE}" "BLOB_OBJECT_STORE_S3_BUCKET" "${BLOB_OBJECT_STORE_S3_BUCKET_VALUE}"
  upsert_env_value "${DEV_ENV_FILE}" "BLOB_OBJECT_STORE_S3_REGION" "${BLOB_OBJECT_STORE_S3_REGION_VALUE}"
  upsert_env_value "${DEV_ENV_FILE}" "BLOB_OBJECT_STORE_S3_ENDPOINT" "${BLOB_OBJECT_STORE_S3_ENDPOINT_VALUE}"
  upsert_env_value "${DEV_ENV_FILE}" "BLOB_OBJECT_STORE_S3_ACCESS_KEY_ID" "${BLOB_OBJECT_STORE_S3_ACCESS_KEY_ID_VALUE}"
  upsert_env_value "${DEV_ENV_FILE}" "BLOB_OBJECT_STORE_S3_SECRET_ACCESS_KEY" "${BLOB_OBJECT_STORE_S3_SECRET_ACCESS_KEY_VALUE}"
  upsert_env_value "${DEV_ENV_FILE}" "BLOB_OBJECT_STORE_S3_FORCE_PATH_STYLE" "${BLOB_OBJECT_STORE_S3_FORCE_PATH_STYLE_VALUE}"
}
...
wait_for_localstack() {
  ...
  while [ "${retries}" -gt 0 ]; do
    status_code="$(curl -s -o /dev/null -w '%{http_code}' "${BLOB_OBJECT_STORE_S3_ENDPOINT}/_localstack/health" || true)"
  ...
  echo "LocalStack did not become healthy at ${BLOB_OBJECT_STORE_S3_ENDPOINT}." >&2
  exit 1
}
...
ensure_bucket_exists() {
  ...
  if [ "${BLOB_OBJECT_STORE_S3_REGION}" = "us-east-1" ]; then
    run_aws_cli s3api create-bucket --bucket "${BLOB_OBJECT_STORE_S3_BUCKET}" >/dev/null
    return
  fi

  run_aws_cli s3api create-bucket \
    --bucket "${BLOB_OBJECT_STORE_S3_BUCKET}" \
    --create-bucket-configuration "LocationConstraint=${BLOB_OBJECT_STORE_S3_REGION}" \
    >/dev/null
}
...
main() {
  ...
  BLOB_OBJECT_STORE_S3_BUCKET="${BLOB_OBJECT_STORE_S3_BUCKET:-tearleads-blobs}"
  ...
  echo "Local S3 is ready at ${BLOB_OBJECT_STORE_S3_ENDPOINT}."
  echo "Bucket: ${BLOB_OBJECT_STORE_S3_BUCKET}"
  echo "Config written to ${DEV_ENV_FILE}."
}

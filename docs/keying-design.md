...
Before promotion, the API hashes the staged object stream and validates the
same binary-envelope parser used by SDK readers: magic, bounded canonical JSON
header, exact fields, suite/version, chunk layout, canonical 12-byte IV, and
total encrypted length. The header buffer is bounded by 64 KiB plus the fixed

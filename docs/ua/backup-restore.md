...
Tearleads надає функціональність резервного копіювання та відновлення для захисту ваших даних. Через специфічні обмеження платформ із зашифрованими базами даних SQLite, формат резервного копіювання та процес відрізняються залежно від платформи.

## Специфікація Формату Файлу TBU

Формат **Tearleads Backup Utility** (`.tbu`) призначений для безпечних і переносимих резервних копій між платформами.
...
flowchart TB
    HEADER["ЗАГОЛОВОК (36 байт, відкритий текст)<br/>Магічні байти: TEARLEADSBAK (12 байт)<br/>Версія формату (2 байти, LE)<br/>Прапори (2 байти, LE)<br/>Сіль (16 байт)<br/>Зарезервовано (4 байти)"]
    CHUNKS["ЗАШИФРОВАНІ CHUNKS<br/>Chunk 0: Manifest<br/>Chunk 1: Database<br/>Chunk 2..N: Blobs (файли)"]

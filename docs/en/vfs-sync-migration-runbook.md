...
- `v024` (backfill legacy staged rows into `vfs_registry + vfs_links`)
- `v025` (drop `vfs_blob_staging` and `vfs_blob_refs`)
- `v026` (drop `vfs_blob_objects` after canonical blob parity check)
- `v027` (drop `vfs_access` after canonical user ACL parity check)
- `v028` (backfill/verify `vfs_shares -> vfs_acl_entries` parity)
- `v029` (backfill/verify `org_shares -> vfs_acl_entries` parity)
- `v030` (backfill/verify `vfs_folders -> vfs_registry` metadata parity)
- `v031` (verify `vfs_folders` retirement preconditions; non-destructive)
- `v032` (record retirement checkpoint snapshot; non-destructive)
- `v033` (drop `vfs_folders` after checkpoint/parity guards)
- `v034` (finalize retirement by dropping checkpoint scaffolding)
- `v035` (record share-retirement parity checkpoint; non-destructive)
- `v036` (verify share-retirement preconditions; non-destructive)
- `v037` (record share-drop planning checkpoint; non-destructive)
- `v038` (record share drop-candidate dry-run checkpoint; non-destructive)
- `v039` (record share pre-drop execution readiness checkpoint; non-destructive)
- `v040` (record share drop-authorization checkpoints; non-destructive)
- `v041` (record share drop execution candidates; non-destructive)
- `v042` (drop `vfs_shares` after executable step-1 guardrails)
- `v043` (drop `org_shares` after step-1 audit + executable step-2 guards)
- `v044` (finalize share retirement by dropping transitional scaffolding)
...
1. Confirm application version includes:
   ...
   - `packages/api/src/migrations/v038.ts`
   - `packages/api/src/migrations/v039.ts`
...
SELECT to_regclass('public.vfs_share_retirement_checkpoints') AS vfs_share_retirement_checkpoints_table;
SELECT to_regclass('public.vfs_share_retirement_drop_plans') AS vfs_share_retirement_drop_plans_table;
SELECT to_regclass('public.vfs_share_retirement_drop_candidates') AS vfs_share_retirement_drop_candidates_table;
SELECT to_regclass('public.vfs_share_retirement_execution_readiness') AS vfs_share_retirement_execution_readiness_table;
SELECT to_regclass('public.vfs_share_retirement_drop_authorizations') AS vfs_share_retirement_drop_authorizations_table;
SELECT to_regclass('public.vfs_share_retirement_drop_execution_candidates') AS vfs_share_retirement_drop_execution_candidates_table;
```
...
1. Migration fails with guardrail exception from `v024` through `v044`.

...
Generated: `2026-03-10T21:12:14.356Z` via `node --experimental-strip-types scripts/bun/generateCompatibilityInventory.ts`.
...
| `@tearleads/api-client` | 11 | vi.resetModules (29), vi.mocked (43), vi.stubEnv (1), DOM setup (2 indicators; see Package Inventory), test script is vitest-primary |
| `@tearleads/api` | 8 | vi.resetModules (9), vi.mocked (4), test script is vitest-primary |
...
| `@tearleads/api-client` | 54 | `vitest-primary` | no | vi.resetModules:29, vi.mocked:43, vi.stubEnv:1 | DOM test environment in vitest config; jsdom dependency | `high-remediation` |
| `@tearleads/api` | 156 | `vitest-primary` | no | vi.resetModules:9, vi.mocked:4 | none | `high-remediation` |

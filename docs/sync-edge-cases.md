...
- Resolved: dormant retained metadata records organization attribution before
  ...
  access edge requests a recursive container crawl; only its successful full
  completion sweeps still-unmatched metadata for that organization. Reattached
  containers clear their markers atomically, failed/partial crawls retain every
  row for retry, and markers belonging to other organizations are not eligible.

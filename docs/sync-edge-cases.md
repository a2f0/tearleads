...
- Resolved: access restoration now prunes stale `access_revoked` container
  ...
  explicitly named removal as a durable observation-generation fence. Recovery
  ...
  and descendants moved elsewhere do not inherit an ancestor's fence.
- Resolved: dormant retained metadata records organization attribution before
  ...
  erasing queued edits or structural intents.

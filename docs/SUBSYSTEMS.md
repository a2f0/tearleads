...
| **Realtime Sync** | Process-local fan-out of Redis pub/sub events to interested sockets: the WS lifecycle, interest index, Redis interest mirror, and upgrade tickets. | `ws.ts` websocket handler wired into the Bun server by `index.ts` | `ws.ts`, `wsRouting.ts`, `wsInterestStore.ts`, `wsTicket.ts`, `wsIdentity.ts` |

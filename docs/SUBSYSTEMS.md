...
| **Identity & Session** | Identity keypairs, auth context, remote encrypted backups, and user signing-key lookup. | `tearleads.identity/session/keyPackageBackups` facades | `client/identity.ts`, `client/identityKeyPackage.ts`, `client/keyPackageBackups.ts`, `client/session.ts`, `client/userKeys.ts` |

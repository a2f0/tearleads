...
| **Identity & Session** | Identity keypairs, auth context, remote encrypted backups, and durable trust-on-first-use validation of complete user identity bundles. | `tearleads.identity/session/keyPackageBackups/userIdentities` facades; `data/trustedUserIdentity` gateway | `client/identity.ts`, `client/identityKeyPackage.ts`, `client/keyPackageBackups.ts`, `client/session.ts`, `client/sessionIdentityTrust.ts`, `client/sessionTypes.ts`, `client/userIdentities.ts`, `data/trustedUserIdentity/` |
...
| **Client Purchases** | Provider-agnostic in-app-purchase capability for org sync billing: the PurchasesCapability interface, the RevenueCat mapping core over an injectable backend, and the unavailable stub. | `createRevenueCatPurchases` / `createUnavailablePurchases` exports | `client/purchases.ts` |

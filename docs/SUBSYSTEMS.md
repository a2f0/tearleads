...
| **Organization Read Models** | Org directory/groups/grants/usage/user-detail read models, roster/profile mutations, and reserved-grant rewrap orchestration for the client. | `tearleads.organizations` facade; `workflows/organizations` | `workflows/organizations/`, `client/organizations.ts`, `client/organizationMetadataReshare.ts`, `client/organizationMetadataReshareCoordinator.ts`, `client/organizationProfileBootstrap.ts`, `client/organizationRootReshare.ts`, `client/organizationRootReshareCoordinator.ts` |
...
| **Identity & Session** | Identity keypairs, auth context, remote encrypted backups, and durable trust-on-first-use validation of complete user identity bundles. | `tearleads.identity/session/keyPackageBackups/userKeys` facades; `data/trustedUserIdentity` gateway | `client/identity.ts`, `client/identityKeyPackage.ts`, `client/keyPackageBackups.ts`, `client/session.ts`, `client/sessionIdentityTrust.ts`, `client/sessionTypes.ts`, `client/userKeys.ts`, `data/trustedUserIdentity/` |

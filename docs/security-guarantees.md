...
container heads current when it was signed, and a dependency path never
replaces the checkpoint-enforced authorizing path recorded for the same leaf.
Whether a link's container evidence predates a later rotation of that
container is the same ordering boundary as for container heads.

...
| **Root Console Client** | Root-only identity, organization, roster, and billing lookups, gated by the session's server-reported root flag. | `tearleads.root` facade; `workflows/root` | `workflows/root/`, `client/root/` |
...
| **Root Console** | The root mini-app: identity and organization directories, live sessions, rosters, and detailed billing/history. Offered only to sessions the server reports as root. | `mini-apps/root` | `mini-apps/root/` |

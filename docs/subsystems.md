...
| **Root Administration** | Internal staff/support surface: the root-identity gate and read-only lookups of identities, activity, sessions, organizations, and billing. | `routes/root` via `createRootRouter`; `services/root` facade; `middleware/root.ts` | `routes/root/`, `services/root/`, `workflows/root/`, `middleware/root.ts` |
...
| **Root Console Client** | Platform-operator identity lookups for the root console, gated locally on the session's server-reported root flag. | `tearleads.root` facade; `workflows/root` | `workflows/root/`, `client/root/` |
...
| **Root Console** | The root mini-app for platform operators: identity listing, identity detail with live sessions, and organization membership with billing standing. Offered only to sessions the server reports as root. | `mini-apps/root` | `mini-apps/root/` |

...
| **SDK Runtime & Composition Root** | The `Tearleads` facade that wires every SDK subsystem object, the runtime-snapshot projector, the SQLite database handle, the events/network state, logging, and the package root entry point. | `new Tearleads(options)`; the package root index | `client/Tearleads.ts`, `client/index.ts`, `client/workflowRuntime.ts`, `client/database.ts`, `client/events.ts`, `client/network.ts`, `client/syncBillingGate.ts`, `client/logger.ts`, `index.ts`, `workflows/runtimeInput.ts` |

...
| **Realtime Sync** | Process-local fan-out of Redis pub/sub events to interested sockets: the WS lifecycle, interest index, Redis interest mirror, and upgrade tickets. | `createRealtimeGateway`, assembled and started by `index.ts` | `realtimeGateway.ts`, `wsConnection.ts`, `wsOrganizationRouting.ts`, `wsRouting.ts`, `wsInterestStore.ts`, `wsTicket.ts`, `wsIdentity.ts` |
...
| **Organization Read Models** | Org directory/groups/grants/usage/user-detail read models, roster/profile mutations, and reserved-grant and group-grant rewrap orchestration for the client. | `tearleads.organizations` facade; `workflows/organizations` | `workflows/organizations/`, `client/organizations.ts`, `client/organizationWorkflowRuntime.ts`, `client/organizationGroupPresentation.ts`, `client/organizationDataUsage.ts`, `client/organizationReadModels.ts`, `client/organizationGroupGrantReshare.ts`, `client/organizationGroupGrantReshare.testFixtures.ts`, `client/organizationGroupGrantReshareRetry.ts`, `client/organizationMetadataReshare.ts`, `client/organizationMetadataReshareCoordinator.ts`, `client/organizationRootReshare.ts`, `client/organizationRootReshareCoordinator.ts` |
| **Principal Policy (client)** | Client-side principal policy workflow operations, admin-signer resolution, and principal-policy crypto helpers. | `workflows/principals` | `workflows/principals/`, `data/organizationAuthorityDescriptor.ts`, `data/principalPolicyAdminSigners.ts`, `data/principalPolicyCrypto.ts` |
...
| **Identity & Session** | Identity keypairs, auth context, and durable trust-on-first-use validation of complete user identity bundles. | `tearleads.identity/session/userIdentities` facades; `data/trustedUserIdentity` gateway | `client/identity.ts`, `client/identityKeyPackage.ts`, `client/session.ts`, `client/sessionIdentityTrust.ts`, `client/sessionTypes.ts`, `client/userIdentities.ts`, `data/trustedUserIdentity/` |
| **Local Keyring** | The on-device keyring (manifest storage, scopes, sessions) and its PIN-code unlock support. | `createLocalKeyring` / `createBrowserLocalKeyring` exports | `client/indexedDbStoreConnection.ts`, `client/localKeyring/`, `client/localKeyring.ts`, `client/localKeyringPinCode.ts`, `client/localKeyringPinCodeSupport.ts` |
| **Client Purchases** | Provider-agnostic purchase capabilities for org sync billing: the `PurchasesCapability` interface (provider-hosted flows) with the RevenueCat mapping core over an injectable backend, the `DirectCheckoutCapability` interface (an in-app payment element the app styles itself), and the unavailable stubs for both. | `createRevenueCatPurchases` / `createUnavailablePurchases` / `createUnavailableDirectCheckout` exports | `client/purchaseErrors.ts`, `client/purchases.ts`, `client/revenueCatCheckoutGate.ts`, `client/revenueCatErrors.ts`, `client/revenueCatIdentity.ts`, `client/revenueCatIdentityTypes.ts`, `client/revenueCatNativeSubscriptionMove.ts`, `client/revenueCatPurchaseErrorNormalization.ts`, `client/revenueCatPurchasePreparation.ts`, `client/revenueCatProviderPhase.ts`, `client/revenueCatProviderSettlement.ts`, `client/revenueCatTimeoutCoordinator.ts`, `client/directCheckout.ts` |

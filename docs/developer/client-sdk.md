...
| `tearleads.root` | platform-operator identity and organization lookups (paged directories and rosters, sessions, detailed billing and the latest 50 billing events), offered only when the session is root |
...
failed local writes, failed lane runs, and thrown container shares retain the
original `Error` for the host's [private diagnostics](./sentry.md), and never
change an outcome. Hosts filter their own catches with the exported
`isDatabaseUnavailableError` and `isProjectionVerificationCancelledError`.
...
refusal through the prepared batch's `carry` step and retry once with the named
rekeys appended, acknowledged with the batch. Custom adapters opt in with
`commitOrganizationGroupPolicyResult`. A container that

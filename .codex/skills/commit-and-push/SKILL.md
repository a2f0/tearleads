...
3. Commit format:
   ...
   - **Header must be ≤ 50 characters** (enforced by commitlint `header-max-length`). The header is the entire first line: `type(scope): description`. Count carefully before committing. If too long, shorten the scope or description:

...
If this Codex session can spawn subagents, dispatch one read-only subagent per
slice in parallel. Each prompt is the shared brief from
[`references/auditor-brief.md`](references/auditor-brief.md) with its
placeholders filled, followed by that slice's section. Verify each report as it
arrives (step 3) instead of waiting for all of them.

Otherwise, audit the slices yourself one at a time. Apply the brief to that
slice's scope only, write its findings to `<scratch>/slice-<name>.md`, and
start the next slice from those notes rather than from memory, so earlier
slices do not crowd out later ones. Report which slices ran and which were
skipped; never skip one silently.

## 3. Verify every finding

Slice findings over-report and mislabel reachability. Nothing reaches the
report without a second look.
...
| Traced | One slice's trace only; never presented as confirmed |
...
1. Print a summary grouped by invariant (parity, A, B) and ranked by severity.
   Give each finding its label, a one-line mechanism, `file:line`, and TLA+
   status. End with what looked sound and the recommended fix order.
2. Draft the issue from
   [`references/issue-template.md`](references/issue-template.md) in the
   scratch directory. After confirmation, or if the user asked for an issue,
   create it with `gh issue create --label bug --body-file <path>`.
...
4. Include the audited revision, the issue number, the verification labels, and
   the fix status in the final summary, so the next audit's known-issues list
   can start there.

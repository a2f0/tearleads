#!/usr/bin/env bash
# Public Electrobun downloads have independent state for each environment.
set -euo pipefail
...
case "$TIER:$ACTION" in
  ...
  *)
    ...
    echo "Production downloads cannot be destroyed through this wrapper." >&2
    exit 1
    ;;
esac
shift 2
...
STACK_DIR="$REPO_ROOT/terraform/stacks/$TIER/downloads"

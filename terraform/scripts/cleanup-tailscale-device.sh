...
TAILNET="$(curl -sf -H "Authorization: Bearer ${TAILSCALE_API_TOKEN}" \
  "https://api.tailscale.com/api/v2/tailnet" | jq -r '.tailnetName // ""')"

if [[ -z "$TAILNET" ]]; then
  echo "Could not resolve tailnet name, skipping device cleanup" >&2
  exit 0
fi

DEVICES="$(curl -sf -H "Authorization: Bearer ${TAILSCALE_API_TOKEN}" \
  "https://api.tailscale.com/api/v2/tailnet/${TAILNET}/devices" | jq -c '.devices // []')"

DEVICE_ID="$(echo "$DEVICES" | jq -r --arg name "$HOSTNAME" \
  '.[] | select(.hostname == $name or .name | endswith("\($name).")) | .id' | head -1)"

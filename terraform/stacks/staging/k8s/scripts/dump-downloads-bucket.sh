...
S3_BUCKET="${S3_BUCKET:-desktop-downloads}"
...
if [[ -z "${DESKTOP_DOWNLOADS_S3_ACCESS_KEY_ID:-}" || -z "${DESKTOP_DOWNLOADS_S3_SECRET_ACCESS_KEY:-}" ]]; then
  echo "ERROR: DESKTOP_DOWNLOADS_S3_ACCESS_KEY_ID and DESKTOP_DOWNLOADS_S3_SECRET_ACCESS_KEY must be set in .secrets/staging.env" >&2
  exit 1
fi
...
echo ""
AWS_ACCESS_KEY_ID="$DESKTOP_DOWNLOADS_S3_ACCESS_KEY_ID" \
AWS_SECRET_ACCESS_KEY="$DESKTOP_DOWNLOADS_S3_SECRET_ACCESS_KEY" \

...
1. **Refuse to move a dirty worktree.** Switching branches with uncommitted
   tracked changes carries in-progress work onto the target branch or strands
   it:

   ```bash
   [ -z "$(git status --porcelain --untracked-files=no)" ] || { echo "Error: worktree has uncommitted changes; not switching branches" >&2; git status --short; exit 1; }
   ```

   Report the dirty paths and **skip steps 2–3**, but still run step 4 — hooks
   live in `.git/hooks`, outside the worktree, so reinstalling them is safe with
   uncommitted work in flight and is usually the reason the reset was asked for.

# Payment Infrastructure Procedure (SOC2)
...
Verify the following metrics are being collected by the observability layer:

| Metric | Expected Values |
| --- | --- |
| outcome | accepted, duplicate, invalid_signature, invalid_payload, misconfigured_secret, unsupported_event_type, replay_window_rejected, processing_error, ingest_error |
| durationMs | Processing time in milliseconds |
| eventType | Normalized event type (uppercase) |
| eventId | RevenueCat event ID |
| organizationId | Resolved organization ID (if applicable) |
| reason | Additional context for rejections/errors |

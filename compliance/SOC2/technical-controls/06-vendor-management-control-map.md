...
| `TL-VENDOR-008` | Anthropic vendor controls | `CLAUDE.md`, `.claude/commands/` | Development session logs |
| `TL-VENDOR-009` | OpenAI vendor controls | `AGENTS.md`, `.codex/skills/` | Development session logs |
| `TL-VENDOR-010` | Google Gemini vendor controls | `.gemini/INSTRUCTIONS.md` | PR review comments |
| `TL-VENDOR-011` | Vendor risk assessment | `compliance/vendor-registry.md` (Risk Assessment Summary) | Risk assessment records |
...
| `packages/api/src/routes/revenuecat/post-webhooks.ts` | Event processing | Idempotent webhook handling |
...
| File | Control | Description |
| --- | --- | --- |
| `packages/api/src/routes/chat/post-completions.ts` | API integration | Authenticated API calls |
